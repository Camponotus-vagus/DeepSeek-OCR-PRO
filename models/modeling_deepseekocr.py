        ...
        if sam_model is not None and (input_ids.shape[1] != 1 or self.training) and torch.sum(images[0][1]).item() != 0:
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                if images_in_this_batch:
                    ...
                    inputs_embeds[idx].masked_scatter_(images_seq_mask[idx].unsqueeze(-1).to(self.device), images_in_this_batch)
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=640, crop_mode=True, test_compress=False, save_results=False, eval_mode=False, max_new_tokens=4096):
        ...
        for text_sep, image in zip(text_splits, images):
            ...
            if crop_mode:
                ...
                images_list.append(image_transform(global_view).to(self.dtype))
                ...
                if width_crop_num > 1 or height_crop_num > 1:
                    ...
                    for i in range(len(images_crop_raw)):
                        images_crop_list.append(image_transform(images_crop_raw[i]).to(self.dtype))
